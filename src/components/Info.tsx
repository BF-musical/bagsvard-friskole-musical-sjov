...
const Info = () => {
  ...
  // Check if specific info sections exist in the data
  const hasPerformances = infoData.performances && infoData.performances.length > 0;
  const hasTickets = infoData.tickets && infoData.tickets.length > 0;
  const hasAddress = infoData.address && 
    (infoData.address.name || infoData.address.street || infoData.address.city);
  const hasPractical = infoData.practical && infoData.practical.length > 0;
                  ...
                  <CardContent className="p-4">
                    <div className="grid grid-cols-2 gap-4">
                      {infoData.tickets.map((ticket, index) => (
                        <div key={index} className={index === 2 && infoData.tickets.length > 2 ? "col-span-2" : ""}>
                  ...
                  {hasAddress && (
                    <>
                      ...
                      <p className="text-gray-700 mb-4">
                        {infoData.address.name && <>{infoData.address.name}<br /></>}
                        {infoData.address.street && <>{infoData.address.street}<br /></>}
                        {infoData.address.city && <>{infoData.address.city}</>}
                      </p>
                    </>
                  )}
                  
                  {hasPractical && infoData.practical.map((item, index) => (
